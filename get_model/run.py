...
class LitModel(L.LightningModule):
    def __init__(self, cfg: DictConfig, train_data_size: int = None):
        super().__init__()
        self.cfg = cfg
        self._train_data_size = train_data_size # Store the passed-in data size
        ...
        # Get data size from the stored attribute
        if self._train_data_size is None:
             raise AttributeError("Training data size was not provided to the LitModel constructor.")
        data_size = self._train_data_size
    ...
    def _shared_step(self, batch, batch_idx, stage="train"):
        ...
        if stage != "predict":
            if isinstance(loss, dict):
                ...
                self.log_dict(
                    loss, batch_size=self.cfg.training.batch_size, sync_dist=distributed
                )
    ...
    def configure_optimizers(self):
        ...
        num_training_steps_per_epoch = (
            data_size // self.cfg.training.batch_size // num_gpu_or_cpu_devices
        )
